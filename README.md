...
## 场景
跨磁盘、跨机器，复制大量小文件，作为 `rsync` 或 `robocopy` 的另外一个选择，读取队列无需等待排队，写入队列极快，只要内存中有文件，就会有线程将其写入磁盘；

当CPU为2015年后的`64位Intel或AMD` CPU时，或Apple 的 `ARM`（M1/M2/M3/M4/M5或更新）时，在`固态硬盘SSD`上，相较于 `rsync`、`robocopy` 较为有明显速度优势；

读取队列会提前预读文件列表、元数据到内存中，写入队列可以节省文件数据的读取时间，当小文件数量到达`5W`以上时，这个累计时间较为客观；

图片中的性能对比数据基于 `33GB` （`14960`个文件）在`windows`实体机、`Mac`实体机、`Linux`虚拟机中测出：
...
# --exclude-dir 默认为 空 ： 如果该文件夹中存在同路径文件，该文件将会被忽略（不会复制到target-dir中去）
# 即 source-dir - exclude-dir = target-dir 中的文件，适用于源文件夹始终不修改，排除的文件置于 exclude-dir 中，要处理的文件在 target-dir 中
#
